...
def generate_superuser(apps, schema_editor):
    """ Create a superuser (admin) if there are no users on the system
        and superuser credentials are found in the environment. This
        is most useful for development.        
    """
    from awaydays.models import User
    if User.objects.count() == 0:
        kwargs = dict(username=os.environ.get("ADMIN_USERNAME"),
                      email=os.environ.get("ADMIN_EMAIL"),
                      password=os.environ.get("ADMIN_PASSWORD"))
        if all(kwargs.values()):
            # If all the values are set, create the superuser
            superuser = User.objects.create_superuser(**kwargs)
            superuser.save()
